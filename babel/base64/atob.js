...
const ascii2base64 = (str) => {
	let baseStr = ''
	const strArr = str.match(/.{1,3}/gs) // 按3位字符为一组切割原字符串。 'ABCD' => 'ABC' 'D'
	if (!strArr) return baseStr

	strArr.forEach((str) => {
		const chars = str.split('') //分割字符串为单个字符  'ABC' => 'A' 'B' 'c'
		const len = str.length

		const charsCodes = chars.map((char) => extraASCIICode(char)) // 将字符转为8位二进制 ‘A’ => 01000001
		console.log('charsCodes:', charsCodes)

		const base64Codes = split8To6(charsCodes) // 将8位二进制分割成6位二进制并补全0
...
const split8To6 = (charsCodes) => {
	const binary6s = charsCodes.join('').match(/[01]{1,6}/g) // 将3组8位二进制拼成24位二进制并切割成4组6位二进制
	if (!binary6s) return []

	const len = charsCodes.length
	switch (len) {
		case 1:
			binary6s[1] = `${binary6s}0000` //  当字符串长度位1 情况1
			break

		case 2:
			binary6s[2] = `${binary6s}00` //  当字符串长度位2 情况2
			break
	}
